...
screen gen_game(screen current){
  int width,height,i,j, rep = 0, *verifligne, t1 = SDL_GetTicks(), t2, finw = 0;
    ...
    while(est_fini(setup) == 0){								/*boucle du jeu*/
        figure = setup.figures[MAX_FIGURES];
        gen_blocks(figure.blocks);						/*génère le bloque*/
        /*ajouter un appel de fonction mlv qui matérialise le block*/
	MLV_draw_rectangle(40, 40, 40, 40, MLV_COLOR_RED);
