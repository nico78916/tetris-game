...
clavier init_clavier(clavier clav,int starty,int size){
	...
	char* a = "AZERTYUIOP\nQSDFGHJKLM\nWXCVBN";
	for(i=0;i<10;i++){
		clav.alpha[i].x = size * i;
		clav.alpha[i].y = starty;
		clav.alpha[i].width = size;
		clav.alpha[i].height = size;
		sprintf(clav.alpha[i].label,"%d", i);
		printf("%s",clav.alpha[i].label);
	}
	while(a[i] != '\0'){
		clav.alpha[i+10].x = size * i;
		clav.alpha[i+10].y = size + starty;
		clav.alpha[i+10].width = size;
		clav.alpha[i+10].height = size;
		sprintf(clav.alpha[i].label,"%d", i);
		printf("%s",clav.alpha[i].label);
	}
	printf("\n");
...
/* Genère le menu pour entrer son pseudo */
screen gen_pseudo(screen current){
    int i,lastY,bw,bh,tw,th,width,height;
    clavier clav;
    char* labels[30] = {"NOUVELLE PARTIE","CONTINUER","CHARGER","OPTIONS","QUITTER"};
    printf("graphics.c    gen_menu\n");
    init_clavier(clav,0,25);
    current.btncount = 5;
    MLV_clear_window(MLV_COLOR_BLACK);
    width = current.width;
    height = current.height;
...
screen gen_game(screen current){
  int height,i,j, finw = 0, count = 0, rnd, couleur;
	...
	printf("\n");
      ...
      for(i=0;i<FIGURE_SIZE;i++){   /*la grilfor(i=0;i<NB_COLS;i++){
	  for(j=0;j<NB_LINES;j++){
	    if(setup.grid[j][i] != 0){
	...
	count++;
	MLV_clear_window(MLV_COLOR_BLACK);
	/* ON génère les blocs avant la grille pour quelle soit déssiné par dessus ! */
	draw_grid(setup);
	for(i = 0;i < NB_COLS;i++){
	  for(j = 0;j < NB_LINES;j++){
